...
class VBScriptGenerator:
    ...
    def convert_to_arduino_script(arduino_mnemonic):
        conversion_rules = {
            "VAR": VBScriptGenerator.convert_var,
            "INF": VBScriptGenerator.convert_inf,
            "REDO": VBScriptGenerator.convert_redo,
            "SET": VBScriptGenerator.convert_set,
            "RUN": VBScriptGenerator.convert_run
        }
    ...
    def convert_var(parts):
        ...
        title_text, msg_text, button, icon = VBScriptGenerator.extract_params(parts[2:])
    ...
    def convert_redo(parts):
        ...
        title_text, msg_text, button, icon = VBScriptGenerator.extract_params(parts[3:])
        ...
        while i < len(params):
            ...
            if key.startswith("TITLE"):
                for j in range(i + 1, len(params)):
                    if params[j].startswith("MSG") or params[j].startswith("BUTTON") or params[j].startswith("ICON"):
                        break
                    value += params[j] + " "
                title_text = value.strip()
            elif key.startswith("MSG"):
                for j in range(i + 1, len(params)):
                    if params[j].startswith("TITLE") or params[j].startswith("BUTTON") or params[j].startswith("ICON"):
                        break
                    value += params[j] + " "
    ...
    def convert_text(self):
        ...
        if not mnemonic_script:
            ...
            for line in mnemonic_script.splitlines():
                converted_line = VBScriptGenerator.convert_to_arduino_script(line.strip())
